...
{
    _tralloc_tests_dynarr * history = user_data;
    if ( history == NULL ) {
        return 0;
    }
    if ( _tralloc_tests_dynarr_append ( history, chunk_context ) != 0 ) {
        return 1;
    }
    return 0;
}
...
tralloc_bool test_destructor_common ( tralloc_context * ctx )
{
    ...
    if (
        tralloc_append_destructor  ( text_01, normal_destructor, history ) != 0 ||
        tralloc_prepend_destructor ( text_02, normal_destructor, NULL )    != 0 ||
        tralloc_prepend_destructor ( text_03, normal_destructor, history ) != 0 ||
        tralloc_append_destructor  ( text_03, normal_destructor, history ) != 0 ||
        tralloc_append_destructor  ( text_04, normal_destructor, NULL )    != 0 ||
        tralloc_prepend_destructor ( text_04, normal_destructor, NULL )    != 0 ||
        tralloc_append_destructor  ( text_04, normal_destructor, history ) != 0 ||
        tralloc_prepend_destructor ( text_04, normal_destructor, history ) != 0
    ) {
        _tralloc_tests_dynarr_free ( history );
        return TRALLOC_FALSE;
    }
    ...
    if (
        ...
        destructor->function != normal_destructor  || destructor->user_data != history || destructor->next != NULL ||
        ...
        destructor->function != normal_destructor  || destructor->user_data != history || ( destructor = destructor->next ) == NULL ||
        destructor->function != normal_destructor  || destructor->user_data != history || destructor->next                  != NULL ||
        destructors->last_destructor != destructor ||
    ...
    if (
        tralloc_append_destructor  ( text_03, empty_destructor_2, history ) != 0 ||
        tralloc_append_destructor  ( text_03, empty_destructor_1, NULL )    != 0 ||
        tralloc_prepend_destructor ( text_03, empty_destructor_2, NULL )    != 0 ||
        tralloc_append_destructor  ( text_03, empty_destructor_2, history ) != 0 ||
        tralloc_prepend_destructor ( text_03, empty_destructor_1, history ) != 0 ||
    ...
    if (
        ...
        destructor->function != empty_destructor_1 || destructor->user_data != history || ( destructor = destructor->next ) == NULL ||
        destructor->function != empty_destructor_2 || destructor->user_data != NULL    || ( destructor = destructor->next ) == NULL ||
        destructor->function != empty_destructor_2 || destructor->user_data != history || ( destructor = destructor->next ) == NULL ||
        destructor->function != empty_destructor_1 || destructor->user_data != NULL    || ( destructor = destructor->next ) == NULL ||
        destructor->function != empty_destructor_2 || destructor->user_data != history || destructor->next                  != NULL ||
        destructors->last_destructor != destructor
    ) {
        _tralloc_tests_dynarr_free ( history );
        return TRALLOC_FALSE;
    ...
    if (
        ...
        ( destructors = _tralloc_get_destructors_from_chunk ( chunk_03 ) ) == NULL || ( destructor = destructors->first_destructor ) == NULL ||
        destructor->function != empty_destructor_1 || destructor->user_data != history || destructor->next != NULL ||
        destructors->last_destructor != destructor
    ) {
        _tralloc_tests_dynarr_free ( history );
        return TRALLOC_FALSE;
    ...
    if (
        ...
        _tralloc_tests_dynarr_get ( history, 0 ) != text_01 ||
        _tralloc_tests_dynarr_get ( history, 1 ) != text_04 ||
        _tralloc_tests_dynarr_get ( history, 2 ) != text_04
    ) {
        _tralloc_tests_dynarr_free ( history );
        return TRALLOC_FALSE;
    }
