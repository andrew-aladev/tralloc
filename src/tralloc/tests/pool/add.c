...
tralloc_bool test_pool_add_strict ( tralloc_context * ctx )
{
    ...
    if ( tralloc_new_with_extensions ( ctx, &pool_data, pool_data_length, TRALLOC_EXTENSION_POOL ) != 0 ) {
        return TRALLOC_FALSE;
    }
...
tralloc_bool test_pool_add_normal ( tralloc_context * ctx )
{
    ...
    if (
        ...
        tralloc_new_with_extensions ( new_ctx, &pool_data, pool_data_length, TRALLOC_EXTENSION_POOL ) != 0 ||
