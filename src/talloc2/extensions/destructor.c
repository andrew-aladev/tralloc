...
static inline
uint8_t delete_destructors ( const talloc_context * chunk_context, destructor_comparator comparator, talloc_destructor_function function, void * user_data )
{
    if ( chunk_context == NULL ) {
        return 1;
    }
...
uint8_t talloc_destructor_free ( talloc_chunk * chunk, talloc_extensions * extensions )
{
    ...
    talloc_context    * chunk_context = talloc_context_from_chunk ( chunk );
    talloc_destructor * destructor    = extensions->first_destructor;
    talloc_destructor * next_destructor;
...
extern inline uint8_t talloc_clear_destructors      ( const talloc_context * chunk_context );
