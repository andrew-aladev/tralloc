...
// Function obtains chunk from chunk_data, detaches it from the tree and deletes subtree starting from it.
// Function returns zero if delete operations were successful and non-zero value otherwise.
// Non-zero value meaning is "the last error code". All memory of subtree will be freed anyway.
inline
...
// Function uses reallocate to change size of chunk from current_length to length.
// Function returns pointer to memory (with length size) or NULL if error occurred.
