...
malloc_dynarr * malloc_history()
{
    ...
    if (
        ...
        tralloc_debug_callback_set_add_data ( NULL, tralloc_history ) != 0
    ) {
        return NULL;
    }
...
tralloc_bool test_debug_add ( tralloc_context * ctx )
{
    ...
    if (
        tralloc_history == NULL ||
        tralloc_debug_callback_set_add_functions ( NULL, after_add ) != 0
    ) {
    ...
    if (
        ...
        tralloc_debug_callback_set_add_functions ( NULL, NULL );
        return TRALLOC_FALSE;
    }
    ...
    if (
        tralloc_free ( a ) != 0 ||
        tralloc_free ( b ) != 0 ||
        tralloc_debug_callback_set_add_functions ( NULL, NULL ) != 0
    ) {
        return TRALLOC_FALSE;
    }
